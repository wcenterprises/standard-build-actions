...
export function getEnvironment(): IEnvironment {
  ...
  return {
    ...
    directories: {
      ...
      artifacts: resolveDirectory(`${path.join(workspace, '../a')}`, {
        create: true
      }),
      staging: resolveDirectory(`${path.join(workspace, '../s')}`, {
        create: true
      }),
      output: resolveDirectory(`${path.join(workspace, '../o')}`, {
        create: true
      }),
      package: resolveDirectory(`${path.join(workspace, '../p')}`, {
        create: true
      }),
      action: resolveDirectory(`${path.join(__dirname, '../../')}`),
      scripts: resolveDirectory(`${path.join(__dirname, '../../scripts')}`),
      temp: resolveDirectory(
        `${path.join(String(process.env['RUNNER_TEMP']), tempDirectry)}`,
        { create: true }
      )
    }
  } as IEnvironment
